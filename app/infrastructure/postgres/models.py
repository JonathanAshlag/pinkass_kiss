...
class PageORM(Base):
    ...
    # Full-text search vector — generated by Postgres via a GENERATED ALWAYS AS expression.
    # Declared as a plain nullable column here so SQLite (used in tests) can create the table.
    # The Alembic migration adds the GENERATED ALWAYS AS clause on real Postgres.
    ...
    __table_args__ = (
        ...
        Index("ix_pages_tags_gin", "tags", postgresql_using="gin"),
    )
...
class AgentORM(Base):
    ...
    active: Mapped[bool] = mapped_column(sa_types.Boolean, nullable=False, default=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False,
        default=lambda: datetime.now(timezone.utc),
    )
    ...
    __table_args__ = (
        Index("ix_agents_api_key_hash", "api_key_hash", unique=True),
    )
