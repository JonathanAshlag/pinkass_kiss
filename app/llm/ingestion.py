...
async def extract_topic_candidates(
    text: str, filename: str, content_parts: list[dict] = None, ingestion_context: str | None = None,
) -> list[dict]:
    """Phase 1: Extract topic titles and descriptions from a document.

    Returns list of {"title": str, "description": str}
    """
    pre, post = build_extract_topics_user(filename, ingestion_context)
...
async def judge_duplicate(
    ...
    return await _call_llm_json(
        messages=[
            ...
            {"role": "user", "content": build_judge_duplicate_user(candidate, search_results, ingestion_context)},
        ],
...
async def generate_page_content(
    title: str, description: str, filename: str, text: str,
    content_parts: list[dict] = None, ingestion_context: str | None = None,
) -> dict:
    """Phase 3A: Generate full wiki page content for a confirmed-new topic.

    Returns {"content": str}
    """
    pre, post = build_generate_content_user(title, description, filename, ingestion_context)
    user_content = _build_multimodal_content(pre, content_parts, post)
...
async def merge_content(
    existing_title: str, existing_content: str, candidate_description: str, filename: str, text: str,
    ...
    return await _call_llm_json(
        messages=[
            ...
            {"role": "user", "content": build_merge_content_user(
                existing_title, existing_content, candidate_description, filename, text, ingestion_context,
            )},
        ],
        name="merge_content",
    )
